<x-app-layout>
    ...
    <div class="py-12"> {{-- Div Principal --}}
        ...
        <form action="{{ route('dashboard') }}" method="GET" class="flex my-4 gap-4 max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <a href="{{ 'gerar-pdf' }}"
                class="bg-yellow-300 dark:bg-yellow-700 hover:bg-yellow-600 dark:hover:bg-yellow-800 text-white font-bold py-2 px-4 rounded">
                GERAR PDF
            </a>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            function adicionarMusicaListaDoDia(botao) {
                ...
                if (listaDoDia) {
                    ...
                    cellVocalista.classList.add('border', 'border-slate-950', 'px-4', 'py-2', 'text-gray-900',
                        'dark:text-black');
                    cellLouvor.classList.add('border', 'border-slate-950', 'px-4', 'py-2', 'text-gray-900',
                        'dark:text-black');
                    cellTonalidade.classList.add('border', 'border-slate-950', 'px-4', 'py-2', 'text-gray-900',
                        'dark:text-black');

                    // Botão de Remover
                    let botaoRemover = document.createElement('button');
                    botaoRemover.textContent = 'Remover';
                    botaoRemover.classList.add('btn-remove', 'px-4', 'py-2', 'bg-red-500', 'text-white', 'rounded',
                        'hover:bg-red-600');
            ...
            function carregarListaDoLocalStorage() {
                ...
                if (listaDeMusicas) {
                    ...
                    listaDeMusicas.forEach(function(musica) {
                        ...
                        let botaoRemover = document.createElement('button');
                        botaoRemover.textContent = 'Remover';
                        botaoRemover.classList.add('btn-remove', 'px-4', 'py-2', 'bg-red-500', 'text-white',
                        ...
                        cellVocalista.classList.add('border', 'border-slate-950', 'px-4', 'py-2',
                            'text-gray-900', 'dark:text-black');
                        cellLouvor.classList.add('border', 'border-slate-950', 'px-4', 'py-2',
                            'text-gray-900', 'dark:text-black');
                        cellTonalidade.classList.add('border', 'border-slate-950', 'px-4', 'py-2',
                            'text-gray-900', 'dark:text-black');
                    });
                }
            }
            ...
            function atualizarBotoesAdicionar() {
                ...
                if (listaDoDia) {
                    ...
                    botoesAddLista.forEach(function(botao) {
                        ...
                        if (louvorJaAdicionado) {
                            botao.style.display =
                                'none'; // Esconde o botão "Add a lista" se a música já estiver na lista
                        } else {
                            botao.style.display =
                                'inline-block'; // Exibe o botão "Adicionar à Lista" se a música não estiver na lista
                        }
                    });
                }
            }
            ...
            botoesAddLista.forEach(function(botao) {
                botao.addEventListener('click', function() {
                    ...
                    botao.style.display =
                        'none'; // Esconde o botão "Adicionar à Lista" após adicionar
                });
            });
